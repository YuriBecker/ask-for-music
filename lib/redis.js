import { Client, Entity, Schema, Repository } from "redis-om";

const client = new Client();

async function connect() {
  if (!client.isOpen()) {
    await client.open(process.env.REDIS_URL);
  }
}

class SongRequest extends Entity {}
class Song extends Entity {}

let SongSchema = new Schema(
  Song,
  {
    artist: { type: "string" },
    name: { type: "string", textSearch: true },
    url: { type: "string" },
    genre: { type: "string" },
  },
  {
    dataStructure: "JSON",
  }
);

export async function addSong(data) {
  await connect();

  const repository = client.fetchRepository(SongSchema);

  const song = repository.createEntity(data);

  const id = await repository.save(song);
  return id;
}

export async function deleteSong(id) {
  await connect();

  const repository = new Repository(SongSchema, client);
  return repository.remove(id);
}

export async function getAllSongs() {
  await connect();

  const repository = new Repository(SongSchema, client);

  const songs = await repository.search().return.all();

  return songs;
}

let SongRequestSchema = new Schema(
  SongRequest,
  {
    requestedBy: { type: "string" },
    description: { type: "string" },
    songId: { type: "string" },
  },
  {
    dataStructure: "JSON",
  }
);

export async function getAuthCode() {
  await connect();

  const code = await client.get("auth-code");

  return code;
}

export async function createSongRequest(data) {
  await connect();

  const repository = client.fetchRepository(SongRequestSchema);

  const songRequest = repository.createEntity(data);

  const id = await repository.save(songRequest);
  return id;
}

export async function getSongRequest(id) {
  await connect();

  const repository = new Repository(SongRequestSchema, client);
  return repository.fetch(id);
}

export async function deleteSongRequest(id) {
  await connect();

  const repository = new Repository(SongRequestSchema, client);
  return repository.remove(id);
}

export async function createIndex() {
  await connect();

  const repository = new Repository(SongSchema, client);
  await repository.createIndex();
}

export async function getAllSongRequests() {
  await connect();

  const repository = new Repository(SongRequestSchema, client);

  const requests = await repository.search().return.all();

  return requests;
}
